...
from library_app.config import (
    ANALYTICS_HTML_FILE,
    ANALYTICS_JS_FILE,
    ANALYTICS_STATIC_CSS_FILE,
    ANALYTICS_STATIC_JS_FILE,
    CAMERA_HTML_FILE,
    CSS_FILE,
    FAVICON_FILE,
    HOST,
...
class LibraryDashboardHandler(BaseHTTPRequestHandler):
    ...
    def _analytics_filters(self):
        ...
        return {
            ...
            "state": params.get("state", ["all"])[0].strip() or "all",
            "row_limit": params.get("row_limit", ["250"])[0].strip() or "250",
        }

    def _analytics_view_mode(self):
        view_mode = self._query_params().get("view", ["masked"])[0].strip().lower()
        return "full" if view_mode == "full" else "masked"
    ...
    def _do_get(self):
        ...
        if path in ("/analytics", "/analytics/"):
            self._html_response(read_text_file(ANALYTICS_HTML_FILE))
            return
        ...
        if path == "/static/analytics.js":
            self._text_response(read_text_file(ANALYTICS_JS_FILE), "application/javascript")
            return
        ...
        # --- Analytics dashboard static assets (served from analytics_dashboard/) ---
        if path == "/analytics-static/styles.css":
            self._text_response(read_text_file(ANALYTICS_STATIC_CSS_FILE), "text/css")
            return

        if path == "/analytics-static/analytics.js":
            self._text_response(read_text_file(ANALYTICS_STATIC_JS_FILE), "application/javascript")
            return

        if path == "/analytics-static/ace_logo.jpeg":
            body = ACE_LOGO_FILE.read_bytes()
            self.send_response(HTTPStatus.OK)
            self.send_header("Content-Type", "image/jpeg")
            self.send_header("Content-Length", str(len(body)))
            ...
            self._json_response(
                {
                    ...
                    "username": credentials["username"] if authenticated else "",
        ...
        if path == "/api/analytics":
            ...
            payload = build_analytics_payload(
                ...
                view_mode=self._analytics_view_mode(),
                row_limit=self._query_params().get("row_limit", ["250"])[0],
                source_token=self._query_params().get("if_token", [""])[0].strip(),
                authenticated=authenticated,
            )
            self._json_response(payload)
            return
        ...
        if path == "/api/analytics-export":
            applied_view, rows = build_analytics_export_rows(
                filters=self._analytics_filters(),
                view_mode=self._analytics_view_mode(),
                authenticated=self._is_authenticated(),
            )
            ...
            body = buffer.getvalue().encode("utf-8")
            filename_suffix = "full" if applied_view == "full" else "masked"
            self._binary_response(
                body,
                "text/csv; charset=utf-8",
                extra_headers={"Content-Disposition": f'attachment; filename="analytics_{filename_suffix}_report.csv"'},
            )
            return
